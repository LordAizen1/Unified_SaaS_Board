...
import { useCohereCosts } from '../../context/CohereCostContext';
...
export const CohereUsage: React.FC = () => {
  ...
  const { billingData, isLoading, error, fetchCohereCosts } = useCohereCosts();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (dateError) return;
    fetchCohereCosts(apiKey, startDate, endDate);
  };
  ...
  return (
    <div className={`p-6 rounded-lg shadow-lg ${theme.isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <h2 className={`flex items-center text-2xl font-bold mb-6 ${theme.isDarkMode ? 'text-white' : 'text-gray-900'}`}>
        <img src={cohereLogo} alt="Cohere Logo" className="w-6 h-6 mr-2" />
        Cohere Usage
      </h2>
      <form onSubmit={handleSubmit} className="mb-6 space-y-4">
        <div>
          <label className={`block text-sm font-medium ${theme.isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Cohere API Key</label>
          <input
            type="password"
            value={apiKey}
