...
import { useGeminiCosts } from '../../context/GeminiCostContext';
...
export const GeminiUsage: React.FC = () => {
  ...
  const { billingData, isLoading, error, fetchGeminiCosts } = useGeminiCosts();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (dateError) return;
    fetchGeminiCosts(apiKey, startDate, endDate);
  };
  ...
  return (
    <div className={`p-6 rounded-lg shadow-lg ${theme.isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <h2 className={`flex items-center text-2xl font-bold mb-6 ${theme.isDarkMode ? 'text-white' : 'text-gray-900'}`}>
        <img src={geminiLogo} alt="Gemini Logo" className="w-6 h-6 mr-2" />
        Gemini Usage
      </h2>
      <form onSubmit={handleSubmit} className="mb-6 space-y-4">
        <div>
          <label className={`block text-sm font-medium ${theme.isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Gemini API Key</label>
          <input
            type="password"
            value={apiKey}
